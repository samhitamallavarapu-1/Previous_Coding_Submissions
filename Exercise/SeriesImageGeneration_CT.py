    ...
    for root, _, files in os.walk(folder_path): # Check all subdirectories
        for file in files:
            if(file.endswith('.dcm')):
                ...
                if(modality in 'CT'):  # Check for CT data
...
image_dir = './Series Images CT' # Folder where images are stored
...
for series_uid, dicom_list in sorted_dicom_series.items():
    ...
    if(rows>1 and cols>1):
        ...
        plt.savefig(f"./{image_dir}/{series_uid}.png", bbox_inches='tight') # Save the figure in Series Images CT folder
    else:
